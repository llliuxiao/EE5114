...
for i = 1:size(output_temp,1)
	...
	for j = 1:particles_count		
        ...
        x = particles(j).x; y = particles(j).y; theta = particles(j).theta;
        dx = motion_estimate(i).x; dy = motion_estimate(i).y; d_theta = motion_estimate(i).theta;

        % Apply rotation to all particles with (additive and proportional) noises 
        d_theta_with_noise = d_theta + randn(1) * theta_noise_proportion * d_theta + randn(1) * theta_noise_add;

        % Apply translation to all particles with (additive and proportional) noises 
        dx_with_noise = dx + randn(1) * translation_noise_proportion * dx + randn(1) * translation_noise_add;
        dy_with_noise = dy + randn(1) * translation_noise_proportion * dy + randn(1) * translation_noise_add;

        particles(j).x = x + dx_with_noise * cos(theta) - dy_with_noise * sin(theta);
        particles(j).y = y + dx_with_noise * sin(theta) + dy_with_noise * cos(theta);
        particles(j).theta = slam_in_pi(theta + d_theta_with_noise);
